...
def clean_pig_timestamps_final():
    # Read the Excel file
    ...
    for idx, row in data_rows.iterrows():
        ...
        for day_num in range(4):  # 0, 1, 2, 3
            ...
            # For day_0, we need a date. For other days, we can use time values
            if day_num == 0:
                ...
                # For days 1-3, check if there's any time data
                has_time_data = False
                ...
                if not has_time_data or base_date is None:
                    continue
...
def create_final_timestamp_csv(pig_timestamps):
    """Create a final CSV with just pig_id, day, and timestamps"""
...
if __name__ == "__main__":
    # Clean the data
    pig_timestamps = clean_pig_timestamps_final()
    
    # Create final CSV
    final_df = create_final_timestamp_csv(pig_timestamps)
    
    # Save to CSV file
    final_df.to_csv('pig_timestamps_final.csv', index=False)
    
    # Print summary
    print(f"Processed {len(pig_timestamps)} timestamp records")
    print(f"Unique pigs: {final_df['pig_id'].nunique()}")
    
    # Show statistics by day
    print(f"\n=== RECORDS BY DAY ===")
    day_counts = final_df['day'].value_counts().sort_index()
    ...
    complete_records = final_df.dropna(subset=['start_time', 'end_time', 'took_off', 'put_on'])
