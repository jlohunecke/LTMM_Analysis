...
def clean_pig_timestamps_with_events():
    # Read the Excel file
...
def create_events_timestamp_csv(pig_timestamps):
    ...
    column_order = ['pig_id', 'day', 'start_time', 'start_event', 'end_time', 'end_event', 'took_off', 'took_off_event', 'put_on', 'put_on_event']
...
if __name__ == "__main__":
    # Clean the data
    pig_timestamps = clean_pig_timestamps_with_events()
    
    # Create events CSV
    events_df = create_events_timestamp_csv(pig_timestamps)
    
    # Save to CSV file
    events_df.to_csv('pig_timestamps_with_events.csv', index=False)
    
    # Print summary
    print(f"Processed {len(pig_timestamps)} timestamp records")
    print(f"Unique pigs: {events_df['pig_id'].nunique()}")
    
    # Show statistics by day
    print(f"\n=== RECORDS BY DAY ===")
    day_counts = events_df['day'].value_counts().sort_index()
    ...
    for col in ['start_time', 'end_time', 'took_off', 'put_on']:
        count = events_df[col].notna().sum()
        print(f"{col}: {count} records")
    
    # Show event statistics
    print(f"\n=== EVENT STATISTICS ===")
    for col in ['start_event', 'end_event', 'took_off_event', 'put_on_event']:
    ...
    event_columns = [col for col in events_df.columns if col.endswith('_event')]
    if event_columns:
        records_with_events = events_df[events_df[event_columns].notna().any(axis=1)]
