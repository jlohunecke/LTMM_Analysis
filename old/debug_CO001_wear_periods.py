...
def plot_CO001_with_wear_periods():
    """Debug function to plot CO-001 with wear periods"""
    ...
    # Print available pig IDs in wear periods
    print("Available pig IDs in wear periods:")
    for pig_id in sorted(wear_periods.keys()):
        print(f"  {pig_id}: {len(wear_periods[pig_id])} periods")
    
    # Check CO-001 specifically
    co001_wear_periods = wear_periods.get('CO-001', [])
    print(f"\nCO-001 wear periods: {len(co001_wear_periods)}")
    for i, (start, end) in enumerate(co001_wear_periods):
        print(f"  Period {i+1}: {start} to {end}")
    
    # Load triaxial data for CO001 (without dash)
    print("\nLoading triaxial data for CO001...")
    triaxial_data = load_triaxial_data('CO001')
    ...
    if triaxial_data is not None:
        ...
        fig.suptitle('CO-001 Triaxial Time Series with Wear Periods (Debug)', fontsize=16, fontweight='bold')
        ...
        print(f"\nAdding {len(co001_wear_periods)} wear period bands...")
        for i, (start_time, end_time) in enumerate(co001_wear_periods):
        ...
        # Save the plot
        plt.savefig('CO001_triaxial_with_wear_periods_debug.png', dpi=300, bbox_inches='tight')
        plt.show()
        
        print("\nPlot saved as CO001_triaxial_with_wear_periods_debug.png")
    else:
        print("No triaxial data available for CO001")

if __name__ == "__main__":
    plot_CO001_with_wear_periods()
