...
def plot_triaxial_with_wear_periods(pig_id_with_dash, triaxial_data, wear_periods):
    """Plot triaxial data with wear periods marked as green bands"""
    
    if triaxial_data is None or len(triaxial_data) == 0:
        print(f"No data available for {pig_id_with_dash}")
        return
    
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(15, 12), sharex=True)
    fig.suptitle(f'Triaxial Time Series for {pig_id_with_dash} with Wear Periods', fontsize=16, fontweight='bold')
    ...
    if pig_id_with_dash in wear_periods:
        wear_periods_for_pig = wear_periods[pig_id_with_dash]
        print(f"Found {len(wear_periods_for_pig)} wear periods for {pig_id_with_dash}")
        
        for i, (start_time, end_time) in enumerate(wear_periods_for_pig):
            ...
            if start_time <= triaxial_data['timestamp'].max() and end_time >= triaxial_data['timestamp'].min():
                print(f"  Adding wear period {i+1}: {start_time} to {end_time}")
    ...
    # Save the plot
    plt.savefig(f'{pig_id_with_dash}_triaxial_with_wear_periods.png', dpi=300, bbox_inches='tight')
    plt.show()

def main():
    """Main function to create plots for CO-001 to CO-010"""
    ...
    # Create plots for CO-001 to CO-010
    for i in range(1, 11):
        pig_id_with_dash = f"CO-{i:03d}"  # Format as CO-001, CO-002, etc. (with dash)
        pig_id_no_dash = f"CO{i:03d}"     # Format as CO001, CO002, etc. (without dash)
        
        print(f"\nProcessing {pig_id_with_dash}...")
        
        # Load triaxial data using the no-dash version
        triaxial_data = load_triaxial_data(pig_id_no_dash)
        
        # Create plot using the with-dash version for wear periods lookup
        plot_triaxial_with_wear_periods(pig_id_with_dash, triaxial_data, wear_periods)
        
        print(f"Plot saved as {pig_id_with_dash}_triaxial_with_wear_periods.png")
