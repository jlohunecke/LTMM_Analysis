...
def plot_triaxial_with_wear_periods(pig_id, triaxial_data, wear_periods):
    """Plot triaxial data with wear periods marked as green bands"""
    
    if triaxial_data is None or len(triaxial_data) == 0:
        print(f"No data available for {pig_id}")
        return
    
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(15, 12), sharex=True)
    fig.suptitle(f'Triaxial Time Series for {pig_id} with Wear Periods', fontsize=16, fontweight='bold')
    ...
    if pig_id in wear_periods:
        for start_time, end_time in wear_periods[pig_id]:
    ...
    # Save the plot
    plt.savefig(f'{pig_id}_triaxial_with_wear_periods.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    for i in range(1, 11):
        pig_id = f"CO{i:03d}"  # Format as CO001, CO002, etc.
        print(f"\nProcessing {pig_id}...")
        
        # Load triaxial data
        triaxial_data = load_triaxial_data(pig_id)
        
        # Create plot
        plot_triaxial_with_wear_periods(pig_id, triaxial_data, wear_periods)
        
        print(f"Plot saved as {pig_id}_triaxial_with_wear_periods.png")
